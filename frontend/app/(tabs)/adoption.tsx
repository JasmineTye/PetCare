 ...
 const fetchUserRole = async () => {
  try {
    ...
    const response = await axios.get(
      `http://192.168.0.167:8080/api/auth/getUserByUserId/${userId}`
    );
...
const handleDelete = async (petId:String) => {
  ...
  try{
    const response = await fetch("http://192.168.0.167:8080/api/adoption/deleteAdoption/"+petId, {
    method: "DELETE",
    headers: {
...
const fetchUsers = async () => {
    setIsLoading(true);
      try {
          ...
          const response = await fetch('http://192.168.0.167:8080/api/adoption/getAllAdoption');
          if (!response.ok) {
